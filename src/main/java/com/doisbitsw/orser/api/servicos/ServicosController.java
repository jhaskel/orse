...
public class ServicosController {
    ...
    public ResponseEntity getEntidadeSetor(@PathVariable("entidade") Long entidade,@PathVariable("setor") Long setor) {
        List<ServicosDTO> carros = service.getEntidadeSetor(entidade,setor);
        return carros.isEmpty() ?
                ResponseEntity.noContent().build() :
                ResponseEntity.ok(carros);
    }

    @GetMapping("/publicos/{entidade}")
    public ResponseEntity getPublico(@PathVariable("entidade") Long entidade) {
        List<ServicosDTO> carros = service.getPublico(entidade);
        return carros.isEmpty() ?
                ResponseEntity.noContent().build() :
                ResponseEntity.ok(carros);
    }
