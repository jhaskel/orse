...
public interface ServicosRepository extends JpaRepository<Servicos, Long> {
    @Query(value = "SELECT ser.*, FROM servicos ser  ORDER BY ser.nome ", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM servicos where entidade = :entidade and setor = :setor ORDER BY nome  ", nativeQuery = true)
    List<Servicos> findEntidadeSetor(Long entidade,Long setor);

    @Query(value = "SELECT * FROM servicos where entidade = :entidade and ispublic = true ORDER BY nome  ", nativeQuery = true)
