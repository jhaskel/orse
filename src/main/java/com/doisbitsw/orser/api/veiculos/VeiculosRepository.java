...
public interface VeiculosRepository extends JpaRepository<Veiculos, Long> {
    ...
    @Query(value = "SELECT * FROM veiculos where setor = :setor and isativo = true and isdisponivel = true  ORDER BY nome ", nativeQuery = true)
    List<Veiculos> findSetor(Long setor);

    @Query(value = "SELECT * FROM veiculos where entidade = :entidade and setor = :setor  ORDER BY isativo desc,isdisponivel desc,nome ", nativeQuery = true)
